...
import Exceptions.InsufficiantAmountException;
...
public class Wallet implements Serializable {
    ...
    public void sell(BigDecimal amount,BigDecimal currentPrice, BankAccount bankAccount) throws InsufficiantAmountException, InvalidAmountException {
        if(amount.compareTo(new BigDecimal("0"))<=0){
            throw new InvalidAmountException();
        }
        BigDecimal reducedAmount = this.amount.subtract(amount);
        if(reducedAmount.compareTo(new BigDecimal("0"))<0) throw new InsufficiantAmountException();
